...
extern int oxnas_eth_initialize(void);
...
static const char* gmac_eth = "gmac";

static int oxnas_phy_read(const char* devname, u8 phyaddr, u8 phyreg, u16* val);
static int oxnas_phy_write(const char* devname, u8 phyaddr, u8 phyreg, u16 phydata);
...
static int oxnas_phy_read(const char* devname, u8 phyaddr, u8 phyreg, u16* val)
{
    ...
    printf("oxnas_phy_read() phyaddr=0x%x, phyreg=0x%x, phydata=0x%x\n", phyaddr, phyreg, data);
...
static int phy_detect(void)
{
    ...
    for (phyaddr = 1; phyaddr < 33; ++phyaddr) {
        ...
        oxnas_phy_read(gmac_eth, phyaddr & 31, MII_PHYSID1, &id1);
        oxnas_phy_read(gmac_eth, phyaddr & 31, MII_PHYSID2, &id2);
...
static void start_phy_reset(void)
{
    ...
    oxnas_phy_write(gmac_eth, priv->phy_addr, MII_BMCR, BMCR_RESET | BMCR_ANRESTART | BMCR_ANENABLE);
}

static int is_phy_reset_complete(void)
{
    int complete = 0;
    u16 bmcr;

    // Read back the status until it indicates reset, or we timeout
    oxnas_phy_read(gmac_eth, priv->phy_addr, MII_BMCR, &bmcr);
    if (!(bmcr & BMCR_RESET)) {
        complete = 1;
    }
...
static void phy_initialise(void)
{
	switch (priv->phy_type) {
		case PHY_TYPE_VITESSE_VSC8201XVZ:
			{
                ...
                u16 acsr;
				oxnas_phy_read(gmac_eth, priv->phy_id, VSC8201_MII_ACSR, &acsr);

				printf("PHY is Vitesse VSC8201XVZ\n");
				acsr |= (1UL << VSC8201_MII_ACSR_MDPPS_BIT);
				oxnas_phy_write(gmac_eth, priv->phy_id, VSC8201_MII_ACSR, acsr);
			}
			break;
		...
		case PHY_TYPE_REALTEK_RTL8211E:
                        printf("PHY is Realtek RTL8211E\n");
                        break;
		...
		case PHY_TYPE_LSI_ET1011C2:
			{
				...
				if (is_phy_interface_rgmii()) {
					...
					oxnas_phy_read(gmac_eth, priv->phy_id, ET1011C_MII_CONFIG, &phy_reg);
					...
					oxnas_phy_write(gmac_eth, priv->phy_id, ET1011C_MII_CONFIG, phy_reg);

					//oxnas_phy_read(gmac_eth, priv->phy_id, ET1011C_MII_CONTROL, &phy_reg);
					//phy_reg |= ET1011C_MII_CONTROL_ALT_RGMII_DELAY;
					//oxnas_phy_write(gmac_eth, priv->phy_id, ET1011C_MII_CONTROL, phy_reg);
				} else {
				...
				// Enable Tx/Rx LED
				oxnas_phy_read(gmac_eth, priv->phy_id, ET1011C_MII_LED2, &phy_reg);
				...
				oxnas_phy_write(gmac_eth, priv->phy_id, ET1011C_MII_LED2, phy_reg);
			}
			break;
        ...
        case PHY_TYPE_ICPLUS_IP1001LF_1:
			...
			if (is_phy_interface_rgmii()) {
                ...
                /* Delay on tx clock */
				oxnas_phy_read(gmac_eth, priv->phy_id, IP1001LF_PHYSPECIFIC_CSR, &phy_reg);
				...
				oxnas_phy_write(gmac_eth, priv->phy_id, IP1001LF_PHYSPECIFIC_CSR, phy_reg);
            }
            break;
        default:
...
{
	u16 lpa, lpa2;
    oxnas_phy_read(gmac_eth, priv->phy_id, MII_LPA, &lpa);
	oxnas_phy_read(gmac_eth, priv->phy_id, MII_STAT1000, &lpa2);
...
{
    u16 val;
    oxnas_phy_read(gmac_eth, priv->phy_addr, MII_BMSR, &val);
    return  val & BMSR_ANEGCOMPLETE;
}

static int is_link_ok(void)
{
    u16 val;
    oxnas_phy_read(gmac_eth, priv->phy_id, MII_BMSR, &val);
	return val & BMSR_LSTATUS;
}

static int oxnas_eth_init(struct eth_device *dev, bd_t* bis)
{
    u32  version;
    u32  reg_contents;
    char  *mac_addr;
    ...
    *(volatile u32*)SYS_CTRL_GMAC_CTRL |= (1UL << SYS_CTRL_GMAC_AUTOSPEED);
    ...
    dev->enetaddr[4] = reg_contents & 0xFF;
    dev->enetaddr[5] = (reg_contents >> 8) & 0xFF;
    ...
    dev->enetaddr[0] = reg_contents & 0xFF;
    dev->enetaddr[1] = (reg_contents >> 8) & 0xFF;
    dev->enetaddr[2] = (reg_contents >> 16) & 0xFF;
    dev->enetaddr[3] = (reg_contents >> 24) & 0xFF;
...
static void oxnas_eth_halt(struct eth_device *dev)
{
    eth_down();
...
static int oxnas_eth_recv(struct eth_device *dev)
{
...
static int oxnas_eth_send(struct eth_device *dev, volatile void *packet, int length)
{
...
int oxnas_eth_initialize(void)
{
    ...
    dev->init = oxnas_eth_init;
    dev->send = oxnas_eth_send;
    dev->recv = oxnas_eth_recv;
    dev->halt = oxnas_eth_halt;
    strcpy(dev->name, gmac_eth);
    
    eth_register(dev);
#if defined(CONFIG_MII)
	miiphy_register(dev->name, oxnas_phy_read, oxnas_phy_write);
#endif
