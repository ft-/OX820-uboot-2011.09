				...
				disk_partition_t * info)
{
	...
	info->start = (lbaint_t) le64_to_int((*pgpt_pte)[part - 1].starting_lba);
	/* The ending LBA is inclusive, to calculate size, add 1 to it */
	info->size = ((lbaint_t)le64_to_int((*pgpt_pte)[part - 1].ending_lba) + 1)
		     - info->start;
					 ...
					 gpt_header * pgpt_head)
{
	...
	if (dev_desc->block_read (dev_desc->dev,
		(lbaint_t)le64_to_int(pgpt_head->partition_entry_lba),
